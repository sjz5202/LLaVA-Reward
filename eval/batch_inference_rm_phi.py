...
def batch_rm_inference(args):
    ...
    if len(dataset[0]) == 3:
        ...
        non_pairwise=False
    dataset = GeneralRewardDataset(dataset,processor=processor,tokenizer=tokenizer, strategy=strategy, is_custom=args.is_custom_dataset, return_prompt_length=False,cls_based=non_pairwise)
    ...
    if not non_pairwise:
        ...
        with torch.no_grad():
            ...
            for inputs_batch_c, inputs_batch_r,c_rates,r_rates in pbar:
                ...
                # print('rewward:',chosen_rewards,reject_rewards)
                end_time = time.time()
