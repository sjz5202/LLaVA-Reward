...
from llava_reward.datasets import GeneralRewardDataset,GeneralRewardDataset_llava
from llava_reward.utils import blending_datasets, get_strategy, get_tokenizer,get_tokenizer_qwen,get_tokenizer_llava,Qwen2RMSNorm,Phi3RMSNorm,LlamaRMSNorm
...
def batch_rm_inference(args):
    ...
    # prepare models
    args,model = load_reward_adaptor(args,model_type='llava',reward_config_path=os.path.join(args.pm_path, "reward_config.yaml"))
    strategy.args = args
    ...
    if len(dataset[0]) == 3:
        ...
        non_pairwise=False
    dataset=GeneralRewardDataset_llava(dataset,processor=processor,tokenizer=tokenizer, strategy=strategy, is_custom=args.is_custom_dataset, return_prompt_length=False,cls_based=non_pairwise)
