...
class Cls_loss(nn.Module):
    ...
    def forward(
        ...
        chosen_labels = torch.ones_like(chosen_reward) 
        reject_labels = torch.zeros_like(reject_reward) 
        chosen_loss = self.bce_loss(chosen_reward, chosen_labels)
        reject_loss = self.bce_loss(reject_reward, reject_labels)

        loss = chosen_loss + reject_loss
        prob = F.sigmoid(chosen_reward - reject_reward)
        return loss.mean(), prob.mean() 

class Binary_Cls_loss(nn.Module):
    """
    Pairwise Loss for Reward Model
    """
    def __init__(self):
    ...
    def forward(
        self, rewards: torch.Tensor, labels: torch.Tensor, margin: torch.Tensor = None
    ) -> torch.Tensor:
        labels = labels.to(torch.bfloat16)
        loss = self.bce_loss(rewards, labels)
        prob = F.sigmoid(rewards)
        return loss.mean(), prob.mean() 
...
class FocalPairWiseLoss(nn.Module):
    ...
    Pairwise Loss for Reward Model
    """
    def __init__(self, gamma:float=0.5 ,tau: float = 0.1):
        super().__init__()
        self.tau = tau
        self.gamma = gamma        
    def forward(
        ...
        if margin is not None:
            loss = -F.logsigmoid((chosen_reward - reject_reward - margin) / self.tau)*(1-F.sigmoid((chosen_reward - reject_reward - margin) / self.tau))**self.gamma
            prob = F.sigmoid((chosen_reward - reject_reward - margin) / self.tau)
        else:
            loss = -F.logsigmoid((chosen_reward - reject_reward) / self.tau)*(1-F.sigmoid((chosen_reward - reject_reward) / self.tau))**self.gamma
            prob = F.sigmoid((chosen_reward - reject_reward) / self.tau)
        return loss.mean(), prob.mean() 
...
class GeneralPreferenceLoss_no_R(nn.Module):
    """
    Loss for General Preference Reward Model
    """
    def __init__(self, tau: float = 1):
    ...
    def forward(
        ...
        if margin is not None:
            result = chosen_reward[:, 0] * reject_reward[:, 0] + chosen_reward[:, 1] * reject_reward[:, 1]
            loss = -F.logsigmoid((result - margin) / self.tau)
            prob = F.sigmoid((result - margin) / self.tau)
        else:
            result = chosen_reward[:, 0] * reject_reward[:, 0] + chosen_reward[:, 1] * reject_reward[:, 1]
