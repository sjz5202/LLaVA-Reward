...
class Phi3ImageEmbedding(nn.Module):
    ...
    def forward(
        ...
        if has_image:
            ...
            img_token_lengths = torch.bincount(positions[0])
            img_token_indices = torch.split(image_features_proj, img_token_lengths.tolist())
            max_img_token_len = max(img_token_lengths)
            img_token_batch_embedding = torch.stack([F.pad(item, (0, 0, 0, max_img_token_len - item.size(0)), "constant", 0) for item in img_token_indices])
        ...
        return hidden_states,img_token_batch_embedding
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if pixel_values is not None and image_sizes is not None:
                ...
                inputs_embeds,vision_embeds = self.vision_embed_tokens(input_ids, pixel_values=pixel_values, image_sizes=image_sizes)
            else:
        ...
        if output_hidden_states:
            all_hidden_states += (hidden_states,)
        all_hidden_states += (vision_embeds,)
        next_cache = None
        if use_cache:
